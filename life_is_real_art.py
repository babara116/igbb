...
with st.spinner("당신의 기분에 딱 맞는 조합을 찾고 있어요... 🔮"):
        try:
            ...
            st.markdown("---")
            st.subheader("▶️ 추천 플레이리스트 YouTube에서 바로 듣기")
            
            # 검색어를 생성해 바로 들을 수 있는 버튼 추가
            search_query = f"{genre} {feel} 노래 추천"
            encoded_query = urllib.parse.quote(search_query)
            youtube_url = f"https://www.youtube.com/results?search_query={encoded_query}"
            
            st.link_button("🎧 YouTube에서 바로 검색해서 들어보기", youtube_url, use_container_width=True)

...
def reset_all():
    ...
    st.session_state.bottom_type = "슬랙스"
...
with col2:
    st.subheader("하의")
    bottom_type = st.radio("종류", ["슬랙스", "트레이닝 팬츠", "반바지", "청바지"], key="bottom_type")
...
if st.button("코디 완성하기"):
    with st.container(border=True):
        ...
        * **매칭 :** {shoes}와 {', '.join(acc) if acc else '악세사리 없이 깔쌈하게!'}
        """)
        st.success("오늘의 스타일링이완성되었습니다! 자신 있게 외출하세요!")

        with st.expander("코디 연출 팁 영상 보기"):
            st.video("https://www.youtube.com/watch?v=rQxBy1Q5wXg")
